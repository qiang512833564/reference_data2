...
- (CDSideBarController*)initWithImages:(NSArray*)images
{
    ...
    for (UIImage *image in [images copy])
    {
        ...
        button.tag = index;
        button.transform = CGAffineTransformTranslate(CGAffineTransformIdentity, 70, 0);//初始的位置
   ...
   // _backgroundMenuView.backgroundColor = [UIColor redColor];
    return self;
}
...
- (void)performOpenAnimation
{
    dispatch_async(dispatch_get_main_queue(), ^{
        [UIView animateWithDuration:0.4 animations:^{
            ...
            _backgroundMenuView.transform = CGAffineTransformTranslate(CGAffineTransformIdentity, -90, 0);//CGAffineTransformTranslate每次都是以传入的transform为起始参照
            // CGAffineTransformIdentity为最初状态，即最初位置的中心点
            //CGAffineTransformTranslate(CGAffineTransformIdentity, -90, 0)从最初位置开始偏移
        }];
    });
    for (UIButton *button in _buttonList)
    {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            [UIView animateWithDuration:0.3f
                                  delay:0.3f
                 usingSpringWithDamping:.7f//使用弹簧和阻尼,相当于弹簧效果，左右波动的范围
                  initialSpringVelocity:.7f//初始速度
                                options:0
                             animations:^{
