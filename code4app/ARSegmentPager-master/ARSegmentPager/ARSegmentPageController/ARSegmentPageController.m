...
-(instancetype)initWithControllers:(UIViewController<ARSegmentControllerDelegate> *)controller, ...
{
    ...
    if (self) {
        ...
        /*
         返回真值，程序继续运行，如果返回假值，则抛出异常，并切可以自定义异常描述
         */
        [self _setUp];//------初始化
        ...
        if (controller)
        {
            ...
            va_start(argumentList, controller);/*获取可变参数列表的第一个参数的地址*/
            
            while ((eachController = va_arg(argumentList, id)))/*获取可变参数的当前参数*/////依次取得所有参数 
            {
                [self.controllers addObject:eachController];
            }
            va_end(argumentList);/*清空va_list可变参数列表*/
        }
    }
    return self;
}
...
{
#if 1
    self.automaticallyAdjustsScrollViewInsets = NO;//设置关闭自动布局
    ...
    /*
     self.edgesForExtendedLayout，extendedLayoutIncludesOpaqueBars这个值只在当前视图有navigationBar或者tabBar时有效
     self.edgesForExtendedLayout = UIRectEdgeNone;设置布局范围,是否在navigationBar的范围上延伸；默认为UIRectEdgeAll全局布局,
     */
