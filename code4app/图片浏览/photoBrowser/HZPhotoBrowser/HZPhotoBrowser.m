...
- (void)showPhotoBrowser
{
    ...
    NSLog(@"%@---%f/n-----%f",NSStringFromCGSize(tempImageView.image.size),placeImageSizeW,placeImageSizeH);
...
- (void)hidePhotoBrowser:(UITapGestureRecognizer *)recognizer
{
    ...
    UIView *sourceView = self.sourceImagesContainerView.subviews[self.currentImageIndex];
    UIView *parentView = [self getParsentView:sourceView];
    CGRect targetTemp = [sourceView.superview convertRect:sourceView.frame toView:parentView];// 将rect由rect所在视图转换到目标视图view中，返回在目标视图view中的rect
    /*
     sourceView.superview---cell
     sourceView.frame-------图片在cell上的位置
     parentView-------------视图控制器的视图
     也就说把图片在cell的frame转化为视图控制器上得frame
     */
