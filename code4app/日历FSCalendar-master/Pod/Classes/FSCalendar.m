...
#define kMonthHeight (822.f*kScale)
const char * flowKey;
...
- (void)layoutSubviews
{
    ...
    if (_scrollView.tag == -1) {
        ...
        _page1.frame = CGRectOffset(_page0.frame, self.flowOffset.x, self.flowOffset.y+kPageSpace);
        _page3.frame = CGRectOffset(_page1.frame, self.flowOffset.x, self.flowOffset.y+kPageSpace);
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    if (offset > self.flowSide) {
       ...
       // NSLog(@"前%f-------%f----------%f",CGRectGetMaxY(_page0.frame),CGRectGetMaxY(_page1.frame),CGRectGetMaxY(_page3.frame));
        _page0.frame = CGRectOffset(_page1.frame, self.flowOffset.x, self.flowOffset.y+kPageSpace);
        _page1.frame = CGRectOffset(_page1.frame, -self.flowOffset.x, -(self.flowOffset.y+kPageSpace));
        _page3.frame = CGRectOffset(_page0.frame, -self.flowOffset.x, -(self.flowOffset.y+kPageSpace));
       ...
       // NSLog(@"前%f-------%f----------%f",CGRectGetMaxY(_page0.frame),CGRectGetMaxY(_page1.frame),CGRectGetMaxY(_page3.frame));
        ...
        _page0.frame = CGRectOffset(_page0.frame, self.flowOffset.x, self.flowOffset.y+kPageSpace);
        _page1.frame = CGRectOffset(_page1.frame, self.flowOffset.x, self.flowOffset.y+kPageSpace);
        _page3.frame = CGRectOffset(_page0.frame, -self.flowOffset.x, -(self.flowOffset.y+kPageSpace));
        _baseOffset -= (self.flowOffset.x + self.flowOffset.y);
...
- (CGSize)flowSize
{
    switch (self.flow) {
        case FSCalendarFlowHorizontal:
            ...
            break;
        case FSCalendarFlowVertical:
            return CGSizeMake(_scrollView.fs_width,  kMonthHeight*3);//_scrollView.fs_height
            break;
        default:
            break;
    }
...
- (CGPoint)flowOffset
{
    switch (self.flow) {
        case FSCalendarFlowHorizontal:
            ...
            break;
        case FSCalendarFlowVertical:
            return CGPointMake(0, kMonthHeight);//_scrollView.fs_height
            break;
        default:
            break;
    }
...
- (CGFloat)flowSide
{
    switch (self.flow) {
        case FSCalendarFlowHorizontal:
            ...
            break;
        case FSCalendarFlowVertical:
            return kMonthHeight;//_scrollView.fs_height;
            break;
        default:
            break;
    }
