...
@implementation HWPostOfficeRecordView
...
{
    /*
     接口：hw-sq-app-web/express/historyList.do 快递历史
     入参：key：用户key
     输出参数：
     {
     "status": "1",
    ...
    [manager POST:KPostOfficeRecordList parameters:param queue:nil success:^(id responese)
     {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    HWPostOfficeRecordCell *cell = [tableView dequeueReusableCellWithIdentifier:cellId];
    if (cell == nil)
    {
        cell = [[HWPostOfficeRecordCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellId];
    }
...
{
    return [HWPostOfficeRecordCell getCellHeight:[self getIndexModel:indexPath]];
}
