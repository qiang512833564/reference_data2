...
//        }];
        ...
        __block UIView *myView = self;
        __block MJPhoto *blockMJPhoto = _photo;
        [_imageView setImageWithURL:_photo.url placeholderImage:_photo.srcImageView.image options:SDWebImageRetryFailed|SDWebImageLowPriority completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType) {
            [Utility hideMBProgress:myView];
            
            if (error) {
                [photoView photoDidFinishLoadWithImage:blockMJPhoto.srcImageView.image];
            }
            else
            {
                [photoView photoDidFinishLoadWithImage:image];
...
-(void)scrollViewDidZoom:(UIScrollView *)scrollView
{
    ...
    if (scrollView.zoomScale < 1)
    {
        ...
        [_imageView setCenter:CGPointMake(scrollView.contentSize.width / 2, window.frame.size.height / 2)];
    }
    else
    {
