...
- (void)popViewControllerWithNewCenterController:(UIViewController *)centerController animated:(BOOL)animated andPresentNewController:(UIViewController *)controllerToPush withDirection:(PPRevealSideDirection)direction andOffset:(CGFloat)offset completion:(void (^)())completionBlock {
    ...
    void (^ bigAnimBlock)(BOOL) = ^(BOOL finished) {
        if (finished) {
            ...
            oldFrame.size.height = [UIScreen mainScreen].bounds.size.height;//#####
...
//        PPRevealSideDirection directionToOpen = [self getSideToClose];
        PPRevealSideDirection directionToOpen = PPRevealSideDirectionLeft;
...
- (void)addShadow {
    ...
    frame.size.height = [UIScreen mainScreen].bounds.size.height - 40.0f;
    ...
    _rootViewController.view.layer.shadowPath = [UIBezierPath bezierPathWithRect:frame].CGPath;
...
- (CGRect)getSlidingRectForOffset:(CGFloat)offset forDirection:(PPRevealSideDirection)direction andOrientation:(UIInterfaceOrientation)orientation {
    ...
    switch (direction) {
        case PPRevealSideDirectionLeft:
            rectToReturn.size = CGSizeMake(_rootViewController.view.frame.size.width, [UIScreen mainScreen].bounds.size.height - 40);
            rectToReturn.origin = CGPointMake(width - offset, 20.0);//###
            break;
        case PPRevealSideDirectionRight:
//            rectToReturn.size = CGSizeMake(_rootViewController.view.frame.size.width, [UIScreen mainScreen].bounds.size.height);
