...
+ (AddressCell *)cellWithTableView:(UITableView *)tableView
{
    ...
    AddressCell * cell = [tableView dequeueReusableCellWithIdentifier:identify];
    if (cell == nil)
    {
        cell = [[AddressCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:identify containingTableView:tableView leftUtilityButtons:nil rightUtilityButtons:nil];
...
- (UIImageView *)selectImage{
    if (_selectImage == nil) {
        ...
        _selectImage.image = [UIImage imageNamed:@"choose_ok2"];
    }
    return _selectImage;
}
...
//    [super selectCell];
    ...
    if (![_tableView isKindOfClass:[HaveAddressTableView class]] )
    {
        for (NSUInteger i = 0; i < array.count; i++) {
            ...
            AddressCell *cell = (AddressCell *)[_tableView cellForRowAtIndexPath:path];
