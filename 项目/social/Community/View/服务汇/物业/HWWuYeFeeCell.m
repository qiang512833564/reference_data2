...
- (void)fillDataWithModel:(HWWuYeFeeModel *)model
{
    ...
    {
        roomStr = [NSString stringWithFormat:@"房号  %@号楼%@单元%@室", model.buildingStr, model.unitNoStr, model.roomStr];
    }
    else
    {
        roomStr = [NSString stringWithFormat:@"房号  %@号楼%@室", model.buildingStr, model.roomStr];
    }
