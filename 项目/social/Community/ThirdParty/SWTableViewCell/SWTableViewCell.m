...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier containingTableView:(UITableView *)containingTableView leftUtilityButtons:(NSArray *)leftUtilityButtons rightUtilityButtons:(NSArray *)rightUtilityButtons
{
    ...
    if (self)
    {
        self.isNeedSelectMode = NO;
        self.selectedBackgroundView = [[UIView alloc]init];
        self.selectedBackgroundView.backgroundColor = THEME_COLOR_TEXTBACKGROUND;
...
- (void)initializer
{
    ...
    UIGestureRecognizer *touchDownGes = [[UIGestureRecognizer alloc] initWithTarget:self action:@selector(scrollViewTouchDown:)];
    touchDownGes.delegate = self;
    [cellScrollView addGestureRecognizer:touchDownGes];
...
#pragma mark - UIGestureRecognizer delegate  //UITapGestureRecognizer代理--影响视图上某些控件的action事件  
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldReceiveTouch:(UITouch *)touch
{
    ...
    {
        if (self.isNeedSelectMode)
        {
            if (gestureRecognizer.state == UIGestureRecognizerStatePossible)
            {
                if (!CGColorEqualToColor(self.contentView.backgroundColor.CGColor, THEME_COLOR_TEXTBACKGROUND.CGColor))
                {
                    self.contentView.backgroundColor = THEME_COLOR_TEXTBACKGROUND;
...
- (void)scrollViewUp:(id)sender
{
    if(_cellState == kCellStateCenter)
    {
        ...
        if ([self.containingTableView.delegate respondsToSelector:@selector(tableView:didSelectRowAtIndexPath:)])
        {
            ...
            [self setSelected:NO];      //原 [self setSelected:YES];
