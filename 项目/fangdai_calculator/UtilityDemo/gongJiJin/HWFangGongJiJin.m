...
@interface HWFangGongJiJin ()<UITableViewDataSource,UITableViewDelegate>
...
@implementation HWFangGongJiJin
...
- (instancetype)init
{
    if(self = [super init])
    {
        ...
        _cellSelectedVC = [[HWFangGongSelectedVC alloc]init];
        
        __unsafe_unretained HWFangGongJiJin *weakSelf = self;
...
-(NSString *)returnRate:(NSString *)year rate:(NSString *)rate index:(NSInteger)indexTemp
{
    ...
    NSString *superRate = [rate stringByAppendingString:@"-2"];
    
    NSArray *arry = [dic_data objectForKey:superRate];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
    if(_temp == 0 )
    {
        ...
        if(_rowCount != _oneDataArray.count)
        {
            [[NSNotificationCenter defaultCenter]postNotificationName:@"cleanNew" object:nil];
        }
        
        _rowCount = _oneDataArray.count;
    ...
    else if(_temp == 1)
    {
        ...
        if(_rowCount != _twoDataArray.count)
        {
            [[NSNotificationCenter defaultCenter]postNotificationName:@"cleanNew" object:nil];
        }
        
        _rowCount = _twoDataArray.count;
...
- (void)jiSuanAction:(UIButton *)btn
{
    ...
    if ([_resultType isEqualToString:@"等额本息按单价计算"]) {
        if (_danJiaValue.length==0 && _mianJiValue.length>0) {
            ...
            //add by gusheng
            [self acumulationUnitPrice:_rateLabel.text];
            //
        }
    }
    ...
    if ([_resultType isEqualToString:@"等额本金按单价计算"]) {
        if (_danJiaValue.length==0 && _mianJiValue.length>0) {
            ...
            [self acumulationUnitBasePrice:_rateLabel.text];
        }
    }
    ...
    if ([_resultType isEqualToString:@"等额本息按总价计算"]) {
        if (_zongEValue.length==0) {
            ...
            //add by gusheng
            [self acumulationTotalPric:_rateLabel.text];
    ...
    if ([_resultType isEqualToString:@"等额本金按总价计算"]) {
        if (_zongEValue.length==0) {
            ...
            [self acumulationTotalPrincSameBase:_rateLabel.text];
        }
    }
