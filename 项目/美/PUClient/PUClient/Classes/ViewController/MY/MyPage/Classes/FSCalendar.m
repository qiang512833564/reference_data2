...
- (void)setMyHeader:(UILabel *)myHeader
{
    ...
    if(_myHeader) {
        ;
        _myHeader.text = [NSString stringWithFormat:@"%ld.%ld.%ld %@",_currentDate.fs_year,_currentDate.fs_month,_currentDate.fs_day,[NSDate fs_dateFromWeek:_currentDate.fs_weekday]];
    }
}
- (void)layoutSubviews
{
...
- (void)handleUnitTap:(FSCalendarUnit *)unit
{
    if ([self shouldSelectDate:unit.date] && !unit.isSelected) {
        ...
        if (_myHeader) {
            ...
            _myHeader.text = [NSString stringWithFormat:@"%ld.%ld.%ld %@",date.fs_year,date.fs_month,date.fs_day,[NSDate fs_dateFromWeek:date.fs_weekday]];
        }
