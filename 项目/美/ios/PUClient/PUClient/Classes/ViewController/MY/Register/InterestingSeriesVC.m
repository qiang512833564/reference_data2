    ...
    [api startWithCompletionBlockWithSuccess:^(YTKBaseRequest *request) {
        ...
        if (dic) {
            ...
            if (json.code == SUCCESSCODE) {
                [IanAlert hideLoading];
                self.sourceArray = json.data[@"series"];
                [self createItem];
...
{
    if (!_mainScrollView) {
        _mainScrollView = [[UIScrollView alloc]initWithFrame:CGRectMake(0, 64, App_Frame_Width, App_Frame_Height - kTopBarHeight)];
...
- (void)createItem
{
    ...
    for (int i = 0; i < self.sourceArray.count; i++) {
        ...
        HotSeriesModel * series = [HotSeriesModel objectWithKeyValues:self.sourceArray[i]];
...
- (void)rightBtnClick {
    ...
    [self performSegueWithIdentifier:@"hotStar" sender:self];
