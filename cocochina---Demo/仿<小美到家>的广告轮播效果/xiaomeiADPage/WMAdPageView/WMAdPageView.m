...
{
#pragma mark  --- 每次刚开始滚动的时候，就会使imageView的frame.origin.x -= frame.size.width * 0.5;即imageView的origin处于其父视图View的-CGRectGetWidth(view.bounds)/2.0f位置，而需要注意的是scrollView的子视图是view，并且该View的位置并没有随着scrollView的滚动而偏移，而仅仅只是View的子视图imageview随着滚动而偏移，并且存在偏移差
    if (scrollView.contentOffset.x > self.frame.size.width) {
        ...
        /*
         _lineView边界线的位置是与scrollView的contentOffset同步的
         */
...
#pragma mark ---- 滚动正在结束和正在减速的时候，迫使scrollView滚动到[_scView scrollRectToVisible:CGRectMake(self.frame.size.width, 0, self.frame.size.width, self.frame.size.height) animated:NO];位置
