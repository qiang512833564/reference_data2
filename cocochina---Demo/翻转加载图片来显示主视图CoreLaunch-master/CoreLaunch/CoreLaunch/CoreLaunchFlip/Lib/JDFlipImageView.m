...
const NSTimeInterval JDFlipImageViewDefaultFlipDuration = 1.6;
...
#pragma mark --------------- 这里是通过截取主视图控制器视图来获得图片 ---------
    self.nextImages = [[JDFlipNumberViewImageFactory sharedInstance]
                       generateImagesFromImage:image];
    // animate
    [self runAnimation];
...
- (void)runAnimation;
{
	[self updateFlipViewFrame];//这里是设置翻转视图的anchorPoint点位置，为了更好的翻转
    
    BOOL isTopDown = (self.flipDirection == JDFlipImageViewFlipDirectionDown);
	
	// setup animation
	CABasicAnimation *animation = [CABasicAnimation animationWithKeyPath:@"transform"];
    animation.duration	= 10;//self.animationDuration/2.0;
...
- (void)updateFlipViewFrame
{
    ...
    if ((isTopDown && self.animationState == JDFlipAnimationStateFirstHalf) ||
		...
		self.flipImageView.layer.anchorPoint = CGPointMake(0.5, 0.0);//使一张图片的瞄点位置移动到指定的位置，以至于显示在视图上的图片的位置有所改变
