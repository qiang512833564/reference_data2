...
-(void)animateTransition:(id<UIViewControllerContextTransitioning>)transitionContext{
    ...
    for (int i = 0; i<self.folds; i++) {
        ...
        leftFromViewFold.layer.position = CGPointMake(offset, size.height*0.5);//这是之所以要设置position是因为在前面更改了属性anchorPoint的值

        [fromViewFolds addObject:leftFromViewFold];
        [leftFromViewFold.subviews[1] setAlpha:1.0];//这里设置alpha为0，是为了使加上过阴影层CAGradientLayer的View暂时不显示
        ...
        [rightFromViewFold.subviews[1] setAlpha:1.0];
    ...
    [UIView animateWithDuration:duration animations:^{
        ...
        for (int i = 0; i < self.folds; i++) {
            ...
            leftFromView.layer.position = CGPointMake(isPresent?0.0:size.width, size.height*0.5);//这里的作用是产生位移移动动画----注意一点将要消失的页面分隔成的四个片段最后都是移动到一个位置
