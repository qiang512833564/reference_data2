...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        self.boxShape.fillRule          = kCAFillRuleEvenOdd;//该参数比较重要

        self.progressShape = [CAShapeLayer layer];
        //self.progressShape.bounds = CGRectMake(0, 0, 80, 80);
        self.progressShape.backgroundColor= [UIColor redColor].CGColor;
...
- (void)layoutSubviews
{
    ...
    self.progressShape.path = [UIBezierPath bezierPathWithRoundedRect:rect
                                                         cornerRadius:radius].CGPath;
    
    NSLog(@"%@",NSStringFromCGRect(rect));
    self.progressShape.lineWidth = 10;
}
